...
#ifdef __XC__
/* XC-interface syntax describing channel protocol used to communicate with control library
...
control_init(void);
   ...
   *
   *  \param c          Array of channel ends used to communicate with controllable entities
   *  \param n          The number of channel ends used
   *
   *  \returns          Whether the registration was successful or not
   */
control_ret_t
control_register_resources(chanend c[n], unsigned n);
...
control_process_xscope_upload(uint8_t buf[], unsigned buf_size,
                              unsigned length_in, unsigned &length_out, chanend c[]);

  /** Inform the control library that the SPI master has ended the transaction. If the command
   *  was a write, this is the point that a write command is emitted on the i_ctl channel end.
   *
   *  \param i_ctl            Array of channel ends used to communicate with controllable entities
   *
   *  \returns                Whether the transfer was successful or not
   */
control_ret_t
control_process_spi_master_ends_transaction(chanend i_ctl[]);

  /** Inform the control library that the SPI master requires data. If the command
   *  was a read, then this is the point that a read command is emitted on the i_ctl channel end.
   ...
   */
control_ret_t
control_process_spi_master_requires_data(uint32_t &data, chanend i_ctl[]);
   ...
   */
control_ret_t
control_process_spi_master_supplied_data(uint32_t datum, uint32_t valid_bits, chanend i_ctl[]);
